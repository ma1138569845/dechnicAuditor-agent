...
> 基础来源：19份省直能源审计报告统计分析。本 skill 组装 Word 时必须遵守。
> 封面 / 基本信息表 / 目录标题 / 正文字体由 `scripts/assemble_report.py`（`ReportGenerator`）落实；
> 目录域与单位名称水印在组装后写入。细则见 `scripts/add_watermark.py`。
...
本 skill 用 `ReportGenerator._add_body_text` 写正文，不要改用 `office_cli_command`。
...
- 时机：组装 `.docx` 落盘后立刻注入（`assemble_report.py` 已调用 `scripts/add_watermark.py`）
