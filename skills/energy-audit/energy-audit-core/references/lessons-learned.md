...
| D10 | **字段"在表里"不等于"在链路里"**：`ts_institution_scene` 的 `bed_num`（床位数）与用能人数四要素 PG 里明明有值（中医医院：在岗 300 / 编外 50 / 门诊 470 / 床位 260），但 `pg_query` 的 SELECT 不查这些列、`pg_collector` 的解析链只有 `('Excel'), ('default')`、全仓 `.py` 零命中 → 医院 5.3.4 床日用水量、人均综合能耗分母只能靠人工补写，`data_sources` 还自相矛盾地标成 `default`/`PG` | 加字段要**三层一起查**：DB 列 → `pg_query` SELECT → `pg_collector` 落 `found` + 解析链有 PG 候选。校验口诀：`data_sources.<字段>=='default'` 而值非 0/非空 ⇒ 一定不是标准链写的（人工补录）。**2026-09-22 已全部接通**：床位 + 用能人数四要素（在岗 work_staff / 编外 logistics_staff / 门诊 clinic_num / 床位 bed_num），医疗机构 `people_count` = 四者之和（`_pg_people_count`），分项落 `base.*` 可审计 | `tools/energy_audit/pg_query.py::get_institution_scene`、`pg_collector.py`（6. 用能场景 + `_pg_people_count`）、`ProjectBase`（work_staff/logistics_staff/clinic_num/beds_count）、`energy-audit-pg-data/SKILL.md`、`standards-values.md`《医院用能人数计算规则》 |
| D11 | **判重方法会骗人**：2026-09-22 技能包审查里，用"整文件 5-gram 包含度"量出 `report-format-spec.md ⊂ docx-techniques.md` 56%、`气候区划摘录 ⊂ standards-values` 43%、`data-model-reference ⊂ chapter-guides-1-4` 35%，据此准备合并/删除；**改按"节"重测后真相相反**——同名节的包含度只有 2~28%（多数 <15%）——短句骨架（"宋体 12pt 居中"、表头、页边距）= 被反复命中，把互补文档抬成"重复" | 判重**必须按节/按语义单元比**，先看两节是不是在回答同一个问题；`report-format-spec`（规范值权威）+ `docx-techniques`（怎么做）是互补，不是重复。**合并重复文档前一律先做分节测量**，否则会把单一权威拆掉 | 测量脚本在**草稿区** `energy-audit-v2/_changes/`（`audit_overlap_detail.py` 分节测量 / `audit_doc_overlap.py` 整文件粗筛）——**不在技能包内**〔草稿区 `energy-audit-v2/_changes/` 已于 2026-09-23 清空，旧文件打包在 `energy-audit-v2/_archive_drafts_20260923.zip`，解压即恢复〕，需要时到该目录取；`energy-audit-core/references/AUTHORITY-INDEX.md` |
| D12 | **"照代码数断言"必然数错**：同一处断言项数，2026-09-22 一天内被写错三次——旧版照打印行数写成"11 项"（当日已扩到 15）；改代码里 `checks[...]` 赋值个数数成"PDF 级 3 项"（其中 `pdf_footer_from_1` **只在失败时才写入**，不是计数项）；原作者写的"15 项 + PDF 级 2 项"本来是对的，反被我"更正"错 | **凡计数/统计类口径，一律以"实跑输出"为准并记录命令与输出**（本处 = `ea_docx_asserts.py <docx> --pdf <pdf>` 打 17 行 `[OK]`）；改别人写对的小数字前，先复现一次 | `ea-validation`… 实为 `energy-audit-report/scripts/ea_docx_asserts.py`（docstring L25-26 只列 2 项 PDF 检查）、`references/script-assembly-chain.md`、`report/SKILL.md` |
...
| W3 | 数据缺失如实标【待补充】却被 V3 判 P0 阻塞交付 | 已登记的数据缺失写入 `<项目>/missing_items.json` → V3 记 **P1 放行**；未登记占位仍 P0 | `ea-validation/SKILL.md`；当时的验证记录在**草稿区** `energy-audit-v2/_changes/T2-直跑验证记录.md`（**不在技能包内**〔草稿区 `energy-audit-v2/_changes/` 已于 2026-09-23 清空，旧文件打包在 `energy-audit-v2/_archive_drafts_20260923.zip`，解压即恢复〕） |
...
| C5 | 用 `robocopy /MIR` 把技能包镜像**整体覆盖** repo `skills/energy-audit`，抹掉目标侧未提交改动（2026-09-20，git 层面不可恢复）。同一天还发现该目录当时有**第二个写入者**在提交，所以目标"又脏又领先" | **`/MIR` 永久禁用**。镜像→repo 一律走 `scripts/deploy_ea_skills.py`：脏文件守卫（目标未提交即停）+ 部署基线守卫（目标内容 ≠ 上次发布记录即停），命中就停、不做任何改动；目标领先用 `--backport` 回流 | `deployment-ops.md`《四》、`scripts/deploy_ea_skills.py`；当时的手工验收脚本在**草稿区** `energy-audit-v2/_changes/test_deploy_guard*.py`（**不在技能包内**〔草稿区 `energy-audit-v2/_changes/` 已于 2026-09-23 清空，旧文件打包在 `energy-audit-v2/_archive_drafts_20260923.zip`，解压即恢复〕） |
