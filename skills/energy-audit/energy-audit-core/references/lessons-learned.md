...
| D11 | **判重方法会骗人**：2026-09-22 技能包审查里，用"整文件 5-gram 包含度"量出 `report-format-spec.md ⊂ docx-techniques.md` 56%、`气候区划摘录 ⊂ standards-values` 43%、`data-model-reference ⊂ chapter-guides-1-4` 35%，据此准备合并/删除；**改按"节"重测后真相相反**——同名节的包含度只有 2~28%（多数 <15%）——短句骨架（"宋体 12pt 居中"、表头、页边距）= 被反复命中，把互补文档抬成"重复" | 判重**必须按节/按语义单元比**，先看两节是不是在回答同一个问题；`report-format-spec`（规范值权威）+ `docx-techniques`（怎么做）是互补，不是重复。**合并重复文档前一律先做分节测量**，否则会把单一权威拆掉 | `_changes/audit_overlap_detail.py`（分节测量脚本）、`_changes/audit_doc_overlap.py`（整文件测量，仅作粗筛）、`energy-audit-core/references/AUTHORITY-INDEX.md` |
