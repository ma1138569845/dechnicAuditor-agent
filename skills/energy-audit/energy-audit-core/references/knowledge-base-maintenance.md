...
- **`energy_audit_technical_guidelines`：13/17 已入库**（2026-09-21）。已入：GB/T 31342-2014、
  发改委国管局令第32号、鲁事管发〔2020〕32号、GB/T 29149-2012、GB/T 2589-2020、
  国务院令第531号、省政府令第210号、GB 17167-**2025**、GB 55015-2021、定额编制应用指南、
  GB/T 32019-2015、GB/T 50034-2024、GB 19577-2024。
  **仍待投递 4 份**：GB/T 51161-2016 民用建筑能耗标准、GB 50176-2016 民用建筑热工设计规范，
  以及**两份需换版本**（用户已给，但文本层不可用）：
  ...
  **教训（新增判据）**：入库前必须抽查"文本层里中文的占比"，不能只看"有文本层"——
  两份都是"有文本层但内容不可用"。实测后果：查"室内空气质量指标限值"只能命中 0.677
