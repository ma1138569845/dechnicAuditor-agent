...
- **`energy_audit_technical_guidelines`：15/17 已入库**（2026-09-21）。已入：GB/T 31342-2014、
  ...
  **教训（已落成工具）**：入库前必须抽查"**文本层里中文的占比**"，不能只看"有文本层"——
  上述两份都是"有文本层但内容不可用"（中文占比 4.5% / 5.4%）。该判据已做成
  `ingest_kb_files.py` 的**硬闸门**：PDF 抽样前 6 页，中文占非空白字符 <30% 直接拒绝
  （`--allow-low-cjk` 可显式放行）。实测后果佐证：查"室内空气质量指标限值"只能命中 0.677
  （GB 55015 顺带提了一句），拿不到 18883 的条文。
