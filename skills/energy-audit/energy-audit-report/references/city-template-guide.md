...
- **version_code 多版本**：is_draft=0 AND deleted=0 的记录可能新旧版并存（PL2026080401/0402），
  同一指标数值可能不同（如 2025 电量 1040085 vs 1011885）。**取数规则：草稿优先（is_draft=1=最新编辑数据），
  无草稿才取 version_code 最大者**（2026-09-04 起版本归一，pg_query 已内置 DISTINCT ON，勿手写"取 version_code 最大"）。
