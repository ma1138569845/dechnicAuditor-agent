...
- `evenAndOddHeaders` 默认关闭；页眉按节引用（2026-09-22 主链定案）：**前置节（封面/信息表/目录）=「仅水印」页眉（无文字、无边框）；正文节=「单位全称 + 报告名 + 分隔线 + 水印」**；节断=目录后「下一页」分节符。
- 交付前自检：`officecli get <报告>.docx /header` 应显示单段落 + `pbdr.bottom=single`，文本无重复单位名。

**覆盖范围**：文字页眉仅显示于**正文节**（第 1 章起）；前置节（封面/信息表/目录）不显示文字页眉（保留水印）。2026-09-22 起与脚本装配主链实现一致。

####  页脚（内容 + 格式）

| 项目                                    | 内容                                                         |
| --------------------------------------- | ------------------------------------------------------------ |
| 前置部分（第 1–3 节，封面/摘要/目录等） | **页脚空白，无页码**（主链 2026-09-22 起实现）                                         |
| 正文部分（第 4 节及之后）               | **居中页码**，仅显示数字（`PAGE` 自动页码域，无“第 X 页”字样），页码编号从1开始（2026-09-22 起为脚本装配主链实现；此前「— N —」版本废止） |
| 字体                                    | **9pt（小五）**，黑色，宋体                                  |
