...
第5章（只渲染表格/图表/数据参考行；分析叙述段由后续 author 补写，你不需要写叙述）:
- 5.1 能耗概况 + 能源流向图
- 5.2 逐类型逐月数据分析 + 图表
- 5.3 五项指标对标表
- 5.4 能耗基准
...
- {CH5}
- {CHTS}/
- chapter_md/ch5_import.md（装配稿就位：运行 prepare_chapter_md.py <项目名>；已存在时保留作者叙述、勿 --force）
...
3. 第2章 公共机构概况（2.1 基本情况/2.2 建筑物概况/2.3 能源资源利用情况；建筑基本信息表每栋一张 4 列键值对：无表头行、全表不加粗，字段与顺序按 chapter-guides-1-4.md §7 v4.0）
4. 第3章 能源资源管理状况
...
- 禁用逐段 doc_insert_paragraph_with_text 写正文（仅封面表模板注入例外）
- 缺失数据标注【待补充】，禁止编造
...
- 禁用逐段 doc_insert_paragraph_with_text 写正文
...
审查内容:
1. 数据一致性:
   - 第2章面积 vs 第5章指标分母（=建筑面积扣除地下车库，D8）：允许相差一个车库面积，其余偏差>1%标记
   - 第4章能耗 vs 第5.2逐类型数据，任一能源差值>5%标记
   - 第5.3非供暖能耗 vs 第4章数据反算值（电×0.31/指标分母面积），偏差>3%标记
   - 第7章问题数量 = 建议数量（每个问题至少一条建议）
2. 章节完整性:
   - 第1章1.6审计依据含省级规章≥3条，文号格式正确
   - 第5章各小节分析叙述非空（5.2 四段式 / 5.3 定义段+结论段 / 5.4 推导段；整节只有图表即阻塞）
   - 第6章按设备类别动态H3，有数据才写
   - 第8章含指标汇总、问题摘要、建议条数
3. 格式规范:
