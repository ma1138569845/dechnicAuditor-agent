...
v1.7.0 重写后，`build_chapter5()` 直接从结构化数据自动生成完整章节（总述+5.1概况+5.3四项指标+5.4基准），全部以 Word 表格方式呈现。
...
| 表号 | 内容 | 列数 | 行数 |
|------|------|------|------|
| 表5.1 | 最新年能源消费结构（能源类型/消耗量/折标/tce/占比） | 6 | 3~6 |
| 表5.2 | 逐年能耗对比（综合能耗/人均/单位面积） | 4 | 4 |
| 表5.3 | 单位建筑面积非供暖能耗（转置） | 4 | 9 |
| 表5.4 | 常规用能系统单位建筑面积电耗（转置） | 4 | 7 |
| 表5.5 | 人均综合能耗（转置） | 4 | 7 |
| 表5.6 | 人均取水量（转置，对标为通用值/先进值） | 4 | 6 |

指标表（表5.3~表5.6）为**行列转换（转置）样式**：首列为指标项（基础数据行 + 指标值行 + 对标值行 + 评价行），
其后每个统计年份一列（2022年/2023年/2024年）；对标值行三年相同仍逐年填充。
行数 = 指标项行数（含表头不计），列数 = 项目列 + 年份数（3年 → 4列）。
| 表5.7 | 能源资源用量基准（有费用数据时生成） | 4 | 2~4 |
| 表5.8 | 能源资源费用基准（有费用数据时生成） | 3 | 2~4 |
