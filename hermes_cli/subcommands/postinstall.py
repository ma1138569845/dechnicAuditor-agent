...
def build_postinstall_parser(subparsers, *, cmd_postinstall: Callable) -> None:
    ...
    postinstall_parser = subparsers.add_parser(
        "postinstall",
        help="Bootstrap non-Python deps for pip installs (node, browser, ripgrep, ffmpeg, officecli)",
        description="One-shot post-install for pip users. Installs system "
        "dependencies that pip cannot provide (plus the optional officecli "
        "preview binary), then runs setup if needed.",
    )
