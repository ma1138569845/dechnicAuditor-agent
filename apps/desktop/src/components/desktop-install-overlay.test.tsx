...
describe('DesktopInstallOverlay first-run setup', () => {
  ...
  it('continues local bootstrap only when Install DechnicAuditor locally is selected', async () => {
    ...
    expect(screen.getByText('Set up DechnicAuditor Desktop')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up DechnicAuditor Desktop')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install DechnicAuditor locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart DechnicAuditor Desktop and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install DechnicAuditor locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart DechnicAuditor Desktop and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install DechnicAuditor locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart DechnicAuditor Desktop and try again.')
    ).toBeTruthy()
    ...
    expect(screen.queryByText('Local installation could not start. Restart DechnicAuditor Desktop and try again.')).toBeNull()
  })
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText('DechnicAuditor needs a one-time install')).toBeTruthy()
