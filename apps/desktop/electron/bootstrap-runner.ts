...
function downloadInstallScript(ref, destPath) {
  ...
  // FORK: DechnicAuditor — self-install must pull the installer from OUR fork
  // (upstream NousResearch/hermes-agent has no rag/ knowledge-base feature).
  const url = `https://raw.githubusercontent.com/ma1138569845/dechnicAuditor-agent/${ref}/scripts/${scriptName}`
