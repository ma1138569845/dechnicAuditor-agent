 ...
 * origin=git@github.com:ma1138569845/dechnicAuditor-agent.git. A background
 ...
 * FORK: the "official" repo is OUR fork (ma1138569845/dechnicAuditor-agent).
 */
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@github.com:ma1138569845/dechnicAuditor-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@github.com:ma1138569845/dechnicAuditor-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/ma1138569845/dechnicAuditor-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/ma1138569845/dechnicAuditor-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:MA1138569845/dechnicAuditor-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/ma1138569845/dechnicAuditor-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@github.com:ma1138569845/dechnicAuditor-agent.git'), true)
  assert.equal(isOfficialSshRemote('git@github.com:ma1138569845/dechnicAuditor-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/ma1138569845/dechnicAuditor-agent.git'), true)
  // Case-insensitive owner/repo match.
  assert.equal(isOfficialSshRemote('git@github.com:MA1138569845/dechnicAuditor-agent.git'), true)
})

test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  // Any OTHER fork over SSH belongs to its user — fetching it is their own
  // remote, not the official (our fork) remote, so the swap must not apply.
  ...
  // HTTPS (to the official repo or upstream) never prompts for SSH/FIDO2, so
  // it keeps the normal fetch path — must not be flagged as an official SSH
  // remote.
