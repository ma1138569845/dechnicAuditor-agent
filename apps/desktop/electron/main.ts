...
// FORK: keep our onlyoffice-config (Office preview/editing) imports; adopt
// upstream's new oauth-partition. Upstream dropped electronProcessStartMarker.
import {
  onlyOfficeStatus,
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || '智能审'
...
if (IS_WINDOWS) {
  app.setAppUserModelId('com.dechnic.dechnicauditor')
}
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  const child = spawnOwnedBackend(
    ...
    hiddenWindowsChildOptions({
      cwd: hermesCwd,
      // FORK: adopt upstream's desktopBackendSpawnEnv wrapper (guest-onboarding
      // stamping) and keep our ONLYOFFICE env injection branch inside the base
      // env object.
...
async function runHermesStart() {
  ...
  const connectionPromise = (async () => {
    ...
    const hermesProcess = spawnOwnedBackend(
      ...
      hiddenWindowsChildOptions({
        cwd: hermesCwd,
        // FORK: adopt upstream's desktopBackendSpawnEnv wrapper and keep our
        // ONLYOFFICE env injection inside the base env object.
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
