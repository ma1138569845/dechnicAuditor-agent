 ...
 *
 * Stamps the Hermes icon + identity onto the packed Windows DechnicAuditor.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'DechnicAuditor')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'DechnicAuditor'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); DechnicAuditor.exe keeps the stock Electron icon`)
  }
}
