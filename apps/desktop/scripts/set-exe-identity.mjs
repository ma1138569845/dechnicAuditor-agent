#!/usr/bin/env node
// set-exe-identity.mjs — stamp the 智能审 icon + version metadata onto the
// built DechnicAuditor.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked DechnicAuditor.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-DechnicAuditor.exe>
//
...
// Stamp the 智能审 icon + identity onto `exe`. Resolves on success, throws on
